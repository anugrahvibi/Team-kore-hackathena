...
export function DamOperatorDashboard() {
  ...
  useEffect(() => {
    async function init() {
      const sData = await fetchSensorReadings('ZONE_ERNAKULAM');
      const aData = await fetchActiveAlerts('dam_controller', '2018_peak');
      const pData = await fetchPredictions('2018_peak');
      
      if (sData.length > 0) setSensor(sData[0]);
      setAlerts(aData);
      setPrediction(pData.find(p => p.zone_id === 'ZONE_ERNAKULAM') || null);
    }
    init();
  }, []);
                    ...
                    <div key={idx} className="p-6 bg-blue-50/5 border border-blue-100/50 rounded-[2.5rem] relative group hover:shadow-lg transition-all flex items-center justify-between gap-6 shadow-sm">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                             <div className={`px-3 py-1 rounded-xl text-[16px] font-black uppercase ${
